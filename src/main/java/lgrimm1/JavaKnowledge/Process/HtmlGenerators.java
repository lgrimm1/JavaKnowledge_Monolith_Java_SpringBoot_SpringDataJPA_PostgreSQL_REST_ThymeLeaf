...
public class HtmlGenerators {
	...
	public List<String> generateMainContent(List<String> text, Formulas formulas, Extractors extractors, TitleRepository titleRepository) {
		...
		while (textIndex < text.size()) {
			...
			if ((text.get(textIndex).equals(formulas.getConstant(Formulas.ConstantName.SUPERLINE))) &&
				...
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a href=\"#top\">Back to top of page</a><br>");
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a href=\""+ formulas.getConstant(Formulas.ConstantName.ROOT_HTML_NAME) +"\">Back to root page</a><br>");
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a name=\"" + text.get(textIndex + 1) + "\"></a>");
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<h2>" + text.get(textIndex + 1) + "</h2>");
				textIndex += 3;
			}
			//header2
			else if ((text.size() > textIndex + 1) && (text.get(textIndex + 1).equals(formulas.getConstant(Formulas.ConstantName.SUBLINE)))) {
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a href=\"#top\">Back to top of page</a><br>");
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a href=\""+ formulas.getConstant(Formulas.ConstantName.ROOT_HTML_NAME) +"\">Back to root page</a><br>");
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a name=\"" + text.get(textIndex) + "\"></a>");
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<h3>" + text.get(textIndex) + "</h3>");
				textIndex += 2;
			}
			//table
			...
			//example
				...
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<h4>" + text.get(textIndex) + "</h4>");
				int textIndex2 = textIndex + 1;
				...
				while (textIndex2 < text.size() && !text.get(textIndex2).equals("END OF EXAMPLE")) {
					html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + text.get(textIndex2) + "<br>");
					textIndex2++;
				}
				html.add(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "</textarea>");
	...
	public String changeToHtmlCharsInLine(String line, Formulas formulas) {
		return line
				...
				.replaceAll(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES), formulas.getConstant(Formulas.ConstantName.TAB_IN_HTML));
	}
	...
	public List<String> collectAndReferenceHeaders(List<String> html, Formulas formulas) {
		...
		for (String header : headers) {
			int headerIndex = newHtml.indexOf(formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + header + "<br>");
			if (headerIndex > -1) {
				newHtml.set(headerIndex, formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + "<a href=\"#" + header + "\">" + formulas.getConstant(Formulas.ConstantName.TAB_IN_SPACES) + header + "</a><br>");
			}
		}
		return newHtml;
	}
